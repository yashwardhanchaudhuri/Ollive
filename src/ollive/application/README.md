...
A focused LLM call first decides whether the current turn depends on prior dialogue; the policy router then selects domain, depth, and explicit web requirements. Grounded turns bind retrieval either to the current user text or to one bounded prior user turn plus the current text. Grounding then enforces structured items, bounded answer length, current-turn marker provenance, and an isolated claim-to-source entailment check before rendering.
